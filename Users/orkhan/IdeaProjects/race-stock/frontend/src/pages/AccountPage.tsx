      ...
      <header className="glass-card rounded-2xl p-6">
        ...
        {user && (
          <div className="mt-4 inline-flex items-center gap-2 rounded-lg border border-brand-500/30 bg-brand-500/10 px-3 py-1.5 text-sm text-brand-100">
            {profile.avatarUrl ? (
              <img src={profile.avatarUrl} alt={user.fullName} className="h-8 w-8 rounded-full border border-brand-400/40 object-cover" />
            ) : (
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-brand-400/40 bg-black/30 text-xs font-semibold text-brand-100">
...
function DebtInfoItem({label, value}: {label: string; value: string}) {
  return (
    <div className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">
      <p className="theme-muted text-[11px] uppercase tracking-wide">{label}</p>
      <p className="theme-heading mt-1 text-sm font-medium">{value}</p>
    </div>
  );
}
