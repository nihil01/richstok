...
type OrdersMode = "history" | "returns";
...
export default function AccountPage({language, user}: AccountPageProps) {
  ...
  const [activeTab, setActiveTab] = useState<AccountTab>(() => resolveAccountTabFromSearch(location.search));
  const [ordersMode, setOrdersMode] = useState<OrdersMode>(() => resolveOrdersModeFromSearch(location.search));
  ...
  useEffect(() => {
    setActiveTab(resolveAccountTabFromSearch(location.search));
    setOrdersMode(resolveOrdersModeFromSearch(location.search));
  }, [location.search]);
  ...
  useEffect(() => {
    ...
    void loadOrders(ordersPageIndex, ordersQuery, ordersMode);
  }, [activeTab, ordersPageIndex, ordersQuery, ordersMode, language]);
  ...
  async function loadProfile() {
    try {
      ...
      setCurrentDebt(normalizeMoney(data.currentDebt));
  ...
  async function loadOrders(pageIndex: number, query: string, mode: OrdersMode) {
    try {
      ...
      const data = await fetchMyOrders({
        ...
        size: 10,
        query,
        scope: mapOrdersModeToScope(mode)
      });
      setOrdersPage(data);
      setOrdersError(null);
  ...
  const ordersSubtitle = useMemo(() => {
    if (ordersMode === "returns") {
      return language === "az"
          ? "Qaytarma üçün uyğun və qaytarılmış sifarişlər."
          : language === "ru"
              ? "Заказы для возврата и уже возвращенные."
              : "Orders eligible for return and already returned.";
    }
    return language === "az"
        ? "Sifariş tarixçəsi (gözləmədə olanlar xaric)."
        : language === "ru"
            ? "История заказов (без ожидающих)."
            : "Order history (excluding pending).";
  }, [ordersMode, language]);
  ...
  return (
      <motion.section initial={{opacity: 0, y: 14}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: -10}} transition={{duration: 0.3}} className="space-y-5">
        <header className="glass-card rounded-2xl p-6">
          ...
          <div className="mt-4 rounded-xl border border-white/12  p-3">
            <p className="theme-muted text-xs uppercase tracking-[0.14em]">{ui.debt.title}</p>
            <div className="mt-2 grid gap-2 sm:grid-cols-1">
              <DebtInfoItem label={ui.debt.currentDebt} value={formatMoneyAzn(currentDebt)} />
            </div>
          </div>
        </header>
        ...
        <AnimatePresence mode="wait">
          {activeTab === "orders" && (
              ...
              >
                ...
                <p className="theme-text mt-1 text-sm">{ordersSubtitle}</p>
        ...
        <div className="space-y-2">
          {details.items.map((item, index) => (
              <div key={`${item.id}-${item.productId ?? "no"}-${index}`} className="rounded-lg border border-white/10 px-3 py-2">
                ...
                {item.customerNote && (
                    <p className="theme-muted mt-1 whitespace-pre-line text-xs">
                      {ui.itemNote}: {item.customerNote}
                    </p>
                )}
...
function resolveAccountTabFromSearch(search: string): AccountTab {
  const tab = new URLSearchParams(search).get("tab");
  if (tab === "profile" || tab === "password" || tab === "orders") {
    return tab;
  }
  return "orders";
}

function resolveOrdersModeFromSearch(search: string): OrdersMode {
  const mode = new URLSearchParams(search).get("mode");
  if (mode === "returns" || mode === "history") {
    return mode;
  }
  return "history";
}

function mapOrdersModeToScope(mode: OrdersMode) {
  if (mode === "returns") {
...
function formatMoneyAzn(value: number) {
  return `${normalizeMoney(value).toFixed(2)} AZN`;
}
